...
state={'score':0,'lives':INITIAL_LIVES,'time_left':GAME_TIME,'game_over':False,'last_time':time.time(),'cat_pos':[0.0,0.0,0.0],'cat_angle':0.0,'cat_height':0.5,'cam_angle':-30.0,'cam_height':7.0,'cam_radius':14.0,'mice':[],'obstacles':[]}
keys_down=set()
...
def draw_cat():
    ...
    # Tail (attached at back, 145° with Y-axis)
    glColor3f(0.0, 0.0, 0.0)
    glPushMatrix()
    glTranslatef(0.0, 0.3, -0.6)  # back of body
    glRotatef(-145, 1, 0, 0)      # 145 degrees with Y-axis
    quad = gluNewQuadric()
    gluCylinder(quad, 0.08, 0.06, 0.9, 10, 1)
...
def draw_obstacle(ob):
    ...
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, [1.0, 0.0, 0.0, 1.0])  # RED material
    glScalef(sx, 1.0, sz)
    glutSolidCube(1.0)
    glPopMatrix()
...
def draw_hud():
 ...
 if state['game_over']: draw_text(WIN_W//2-140,WIN_H//2,"GAME OVER-Press R to Restart")
 glEnable(GL_LIGHTING)
 glPopMatrix()
 glMatrixMode(GL_PROJECTION)
 glPopMatrix()
...
def display():
 ...
 gluLookAt(catx+cam_x,state['cam_height'],catz+cam_z,catx,0.8,catz,0,1,0)
 glEnable(GL_LIGHTING)
 glEnable(GL_LIGHT0)
...
def keyboard(key,x,y):
 if isinstance(key,bytes): k=key.decode('utf-8')
 else: k=str(key)
 k=k.lower()
